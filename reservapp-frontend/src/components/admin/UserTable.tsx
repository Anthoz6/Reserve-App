...
export default function UserTable({
    ...
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          {hasFilters ? "Intenta ajustar los filtros de búsqueda" : "Comienza creando tu primer usuario"}
        </p>
  ...
  return (
    <>
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                <td className="py-4 px-6">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-md bg-primary/10 flex items-center justify-center">
                      <span className="text-sm font-medium text-primary">{user.name.slice(0, 1).toUpperCase()}</span>
                    </div>
                    <div>
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${user.enabled ? "bg-chart-4" : "bg-muted-foreground"}`} />
                    <span className={`text-sm ${user.enabled ? "text-chart-4" : "text-muted-foreground"}`}>
                  ...
                  <div className="flex items-center justify-end">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" disabled={actionLoading === user.id}>
                          <MoreHorizontalIcon className="w-4 h-4" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={() => router.push(`/dashboard/admin/users/edit?id=${user.id}`)}>
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open: boolean) => setDeleteDialog({ open, user: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
