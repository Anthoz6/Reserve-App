...
function CustomerServicesContent() {
  ...
  const [reservationDialog, setReservationDialog] = useState<{ open: boolean; service: Service | null }>({
    open: false,
    service: null,
  })
      ...
      setFilteredServices(
        services.filter(
          (service) =>
            ...
            (service.provider?.name && service.provider.name.toLowerCase().includes(searchTerm.toLowerCase()))
        )
      )
    } else {
  ...
  const handleSubmitReservation = async () => {
    ...
    try {
      ...
      const reservationData = {
        ...
        time: reservationForm.time.includes(":00") ? reservationForm.time : reservationForm.time + ":00"
      }
      ...
      if (err.response?.status === 400) {
        ...
        let errorMessage = "Datos de reserva inválidos"
        ...
        if (errorData?.details && Array.isArray(errorData.details) && errorData.details.length > 0) {
          ...
          if (typeof firstDetail === 'string') {
            ...
            errorMessage = `${firstDetail.field}: ${firstDetail.defaultMessage}`
          }
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {filteredServices.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredServices.map((service) => (
            <div key={service.id} className="bg-card border rounded-lg overflow-hidden hover:shadow-sm transition-shadow">
              <div className="p-6">
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    <div className="text-sm font-medium">{service.provider?.name || "Proveedor"}</div>
                    <div className="text-xs text-muted-foreground">{service.provider?.email || "Sin email"}</div>
                  </div>
                </div>

                <Button
                  onClick={() => handleReserveService(service)}
                  className="w-full"
                >
      ...
      {/* Reservation Dialog */}
      <Dialog open={reservationDialog.open} onOpenChange={(open) => setReservationDialog({ open, service: null })}>
            ...
            {reservationDialog.service && (
              <div className="bg-muted/50 rounded-lg p-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">${reservationDialog.service.price.toFixed(2)}</span>
                </div>
              </div>
            )}
          </div>
